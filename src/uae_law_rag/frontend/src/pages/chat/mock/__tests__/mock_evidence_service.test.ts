...
describe('mock evidence service', () => {
  it('filters and paginates retrieval hits', async () => {
    ...
    const firstPage = await service.getRetrievalHits({ source: 'keyword', limit: 5, offset: 0 })
    ...
    const secondPage = await service.getRetrievalHits({ source: 'keyword', limit: 5, offset: 5 })
